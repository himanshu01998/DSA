...
# If user wants to sort dictionary based on First Key 'A', Then Key 'B', they shall pass list 
# of keys in the order of preference as a list ['A','B']. Your code should be able to sort 
# list of dictionaries for any number of keys in sorting order list.

# Using this multi-level sort, you should be able to sort any list of dictionaries based 
# on sorting order preference
...
def selection_sort(arr, keys):
    ...
    for i in range(size-1):
        ...
        for j in range(min_index+1, size):
            if arr[j][keys[0]] == arr[min_index][keys[0]]:
                if arr[j][keys[1]] < arr[min_index][keys[1]]:
                    min_index = j
                
            elif arr[j][keys[0]] < arr[min_index][keys[0]]:
                min_index = j
